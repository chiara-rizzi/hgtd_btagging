...
leg1="ITk"
leg2="HGTD Initial"
leg3="HGTD Int. pre-repl."
leg4="HGTD Int. post-repl."
leg5="HGTD Final"
...
for tag in taggerList:
    ...
    f5.cd()
    if name5!="": tag=name5
    curve5=f5.Get(tag+"---bl")
    if curve5==None: continue
    print curve
    print curve2
    print curve3
    print curve4
    ...
    curve4.SetLineColor(kRed)
    curve4.SetLineWidth(3)
    curve4.Draw("C")

    curve5.SetLineStyle(1)
    curve5.SetLineColor(kYellow)
    curve5.SetLineWidth(3)
    curve5.Draw("C")
    ...
    if not (name1!="" and name2!=""):
        ...
        legend5.AddEntry(curve5,leg5,"L")
    else:
        ...
        legend5.AddEntry(curve5,leg2+" : "+name3,"L")
    legend5.Draw("SAME")
    ...
    if True:#name5=="":
        ...
        for bin in xrange(1,curve.GetN()+1):
        	...
        	#if clo_py2!=100000 : ratC.SetPoint(bin-1,px1,clo_py2/py1)
        	#else               : ratC.SetPoint(bin-1,px1,-1)
            clo_py2=curve5.Eval(px1)
            if clo_py2!=100000 :
                print " ---> filling point: "+str(countPoint)+"  with: "+str(clo_py2/py1)
            	ratC4.SetPoint(countPoint,px1,clo_py2/py1)
                #	if "Errors" in ratC.ClassName(): ratC.SetPointError(countPoint,0,0)
                #ratC.SetPointError(countPoint,0,0)
            	countPoint+=1

        for point in xrange(countPoint+1,ratC4.GetN()+1): ratC4.RemovePoint(point)
        ratC4.SetLineWidth(2)
        ratC4.SetLineColor(kYellow)
        ratC4.Draw("SAMEL") 
        # chiara: here it ends
